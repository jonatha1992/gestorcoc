...
async function migrate() {
    ...
    for (let i = 0; i < rawData.length; i++) {
        ...
        const record = {
            orderNumber: row['Nº ORDEN']?.toString() || '',
            entryDate: formatDate(row['FECHA INGRESO']),
            requestTypeId: await getOrCreateItem(CATALOG_CODES.TIPOS_SOLICITUD, row['TIPO \r\nSOLICITUD']),
            requestNumber: row['NUMERO SOLICITUD']?.toString() || '',
            requester: row['SOLICITANTE']?.toString() || '',
            judicialCase: row['Nº DE CAUSA JUDICIAL/PREVENCION SUMARIA']?.toString() || '',
            caratula: row['CARATULA']?.toString() || '',
            incidentDate: formatDate(row['FECHA DEL HECHO']),
            crimeTypeId: await getOrCreateItem(CATALOG_CODES.TIPOS_DELITO, row['TIPO DE DELITO']),
            unitId: await getOrCreateItem(CATALOG_CODES.UNIDADES, row['DEPENDENCIA INTERVINIENTE']),
            receivedBy: row['RECEPCIONADO POR ']?.toString() || '',
            madeBy: row['CONFECCIONADO POR']?.toString() || '',
            detail: row['DETALLE']?.toString() || '',
            dvdNumber: row['Nº DE DVD']?.toString() || '',
            reportNumber: row['Nº DE INFORME']?.toString() || '',
            ifgra: row['IFGRA']?.toString() || '',
            fileNumber: row['EXPEDIENTE']?.toString() || '',
            deliveryAct: row['Nº ACTA DE ENTREGA / ELEVACION']?.toString() || '',
            exitDate: formatDate(row['FECHA DE SALIDA']),
            withdrawnBy: row['RETIRADO POR']?.toString() || '',
            organizationId: await getOrCreateItem(CATALOG_CODES.ORGANISMOS, row['ORGANISMO']),
            status: statusMap[row['ESTADO']] || 'Pendiente',
            observations: row['OBSERVACIONES']?.toString() || '',
            title: `Registro ${row['Nº ORDEN'] || i}`,
            createdAt: serverTimestamp(),
            createdBy: 'migration_script'
        };
