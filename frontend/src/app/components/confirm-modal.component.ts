    ...
    template: `
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm"
            ...
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden transform transition-all animate-fade-in"
                ...
                <!-- Body -->
                <div class="px-6 py-5">
                    <p class="text-sm text-slate-600 leading-relaxed">{{ options.message }}</p>
                    
                    @if (options.showWarning && options.warningText) {
                    <div class="mt-4 p-3 bg-rose-50 border border-rose-200 rounded-lg">
                        <p class="text-xs text-rose-700 font-medium">{{ options.warningText }}</p>
                    </div>
                    }
                </div>
...
export class ConfirmModalComponent {
    ...
    getConfirmButtonClass(): string {
        return this.options.confirmButtonClass || 'text-white bg-rose-600 hover:bg-rose-700 focus:ring-rose-500';
    }
}
